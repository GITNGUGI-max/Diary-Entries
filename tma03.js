...
function processFile(changeEvent) {
    ...
    function addImage(dataUrl) {
        ...
        // Store the item in local storage using the given key
        // (demonstrated elsewhere in this file)
        const imgPath = document.querySelector("#image input").files[0];
        const reader = new FileReader();

        reader.addEventListener("load", function () {
            // convert image file to base64 string and save to localStorage
            localStorage.setItem(itemKey, reader.result);
        }, false);
    ...
    const imgPath = document.querySelector('input[type="file"]');

        
            imgPath.addEventListener("change", function(e){
                const reader = new FileReader();
